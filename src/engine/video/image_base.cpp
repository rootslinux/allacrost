...
// segfaults when ImageMemory objects were copied via copy constructor.
	if (pixels != nullptr) {
		IF_PRINT_WARNING(VIDEO_DEBUG) << "pixels member was not nullptr upon object destruction" << endl;
//		free(pixels);
//		pixels = nullptr;
	}
}
...
bool ImageMemory::LoadImage(const string& filename) {
	if (pixels != nullptr) {
		IF_PRINT_WARNING(VIDEO_DEBUG) << "pixels member was not nullptr upon function invocation" << endl;
		free(pixels);
		pixels = nullptr;
	}
...
bool ImageMemory::SaveImage(const string& filename, bool png_image) {
	if (pixels == nullptr) {
		IF_PRINT_WARNING(VIDEO_DEBUG) << "pixels member was nullptr upon function invocation for file: " << filename << endl;
		return false;
	}
...
void ImageMemory::ConvertToGrayscale() {
	...
	if (pixels == nullptr) {
		IF_PRINT_WARNING(VIDEO_DEBUG) << "no image data (pixels == nullptr)" << endl;
		return;
	}
