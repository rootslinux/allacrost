	...
	const uint32 grid_rows, const uint32 grid_columns)
{
	...
	for (uint32 i = 0; i < images.size(); i++) {
		if (images[i] == nullptr || images[i]->_image_texture == nullptr) {
			IF_PRINT_WARNING(VIDEO_DEBUG) << "nullptr StillImage or ImageElement was present in images vector argument when saving file: " << filename << endl;
			return false;
		}
...
void ImageDescriptor::_RemoveTextureReference() {
	if (_texture == nullptr) {
		IF_PRINT_WARNING(VIDEO_DEBUG) << "_texture member was nullptr upon method invocation" << endl;
		return;
	}
...
void ImageDescriptor::_GetPngImageInfo(const std::string& filename, uint32& rows, uint32& cols, uint32& bpp) throw(Exception) {
	...
	if (!info_ptr) {
		png_destroy_read_struct(&png_ptr, nullptr, (png_infopp)nullptr);
		fclose(fp);
		throw Exception("png_create_info_struct() returned nullptr for file: " + filename, __FILE__, __LINE__, __FUNCTION__);
		return;
	}
...
bool StillImage::Load(const string& filename) {
	...
	if ((_image_texture = TextureManager->_GetImageTexture(_filename)) != nullptr) {
		...
		if (_image_texture == nullptr) {
			IF_PRINT_WARNING(VIDEO_DEBUG) << "recovered a nullptr image inside the TextureManager's image map: " << _filename << endl;
			return false;
		}
...
bool AnimatedImage::AddFrame(const StillImage& frame, uint32 frame_time) {
	...
	if (frame._image_texture == nullptr) {
