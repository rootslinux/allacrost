...
template<> hoa_video::TextureController* Singleton<hoa_video::TextureController>::_singleton_reference = nullptr;
...
bool TextureController::UnloadTextures() {
	...
	while (i != _tex_sheets.end()) {
		if (*i != nullptr) {
			...
			IF_PRINT_WARNING(VIDEO_DEBUG) << "a nullptr TextureSheet was found in the _tex_sheets container" << endl;
			success = false;
		}
	...
	while (j != TextManager->_font_map.end()) {
		...
		if (fp->glyph_cache != nullptr) {
			for (uint32 k = 0; k < fp->glyph_cache->size(); ++k) {
				if (fp->glyph_cache->at(k) != nullptr) {
...
void TextureController::DEBUG_ShowTexSheet() {
	...
	if (sheet == nullptr) {
		IF_PRINT_WARNING(VIDEO_DEBUG) << "discovered a nullptr texture sheet in _tex_sheets container" << endl;
		return;
	}
...
void TextureController::_RemoveSheet(TexSheet* sheet) {
	if (sheet == nullptr) {
		IF_PRINT_WARNING(VIDEO_DEBUG) << "nullptr argument passed to function" << endl;
		return;
	}
...
TexSheet* TextureController::_InsertImageInTexSheet(BaseTexture *image, ImageMemory& load_info, bool is_static) {
	...
	for (uint32 i = 0; i < _tex_sheets.size(); i++) {
		...
		if (sheet == nullptr) {
			IF_PRINT_WARNING(VIDEO_DEBUG) << "found a nullptr texture sheet in the _tex_sheets container" << endl;
			continue;
		}
		...
		if (is_multi_image) {
			...
			if (multi_image_info.find(img->filename) == multi_image_info.end()) {
				...
				if (image.pixels == nullptr) {
					IF_PRINT_WARNING(VIDEO_DEBUG) << "call to malloc returned nullptr" << endl;
					success = false;
					continue;
				}
