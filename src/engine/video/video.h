...
public:
	...
	**/
	...
	/** \brief Used to set the state indicating that other code drawing graphics should display any relevant debugging visuals
	*** \param debug Whether to enable or disable graphical debugging
	**/
	void DEBUG_SetGraphicsDebuggingEnabled(bool debug)
		{ _DEBUG_graphics_debugging_enabled = debug; }

	/** \brief Instructs code drawing graphics with this engine should additionally draw any debugging graphics
	*** \return True if debugging graphics should be drawn
	**/
	bool DEBUG_IsGraphicsDebuggingEnabled() const
		{ return _DEBUG_graphics_debugging_enabled; }
private:
	VideoEngine();

	/** \brief Not used by the engine directly, but rather indicates to code using the engine that they should display relevant debugging graphics
	*** This is used, for instance, by the GUI system to draw outlines of GUI elements. The MapMode class uses it to show the collision grid and
	*** collision rectangles of on-screen sprites. And so on. Typically, this variable gets set in the input engine by detecting a special key press.
	**/
	bool _DEBUG_graphics_debugging_enabled;

	//! \brief fps display flag. If true, FPS is displayed
	bool _fps_display;
