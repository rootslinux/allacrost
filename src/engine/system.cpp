...
template<> hoa_system::SystemEngine* Singleton<hoa_system::SystemEngine>::_singleton_reference = nullptr;
...
void SystemEngine::AddAutoTimer(SystemTimer* timer) {
	if (timer == nullptr) {
		IF_PRINT_WARNING(SYSTEM_DEBUG) << "function received nullptr argument" << endl;
		return;
	}
