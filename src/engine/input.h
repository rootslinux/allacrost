...
/** \brief Enumerated values of the player input commands
*** This list only includes those commands that are standard inputs and does not include hard-key mappings
*** like F1 for help or Ctrl+Q to quit.
**/
enum INPUT_STANDARD_COMMAND {
	COMMAND_INVALID        = -1,
	UP_COMMAND             = 0,
	DOWN_COMMAND           = 1,
	LEFT_COMMAND           = 2,
	...
	MENU_COMMAND           = 6,
	SWAP_COMMAND           = 7,
	LEFT_SELECT_COMMAND    = 8,
	RIGHT_SELECT_COMMAND   = 9,
	PAUSE_COMMAND          = 10,
	COMMAND_TOTAL          = 11
};
...
***
*** This class is a container for various SDLKey structures that represent
...
public:
	//! \brief A pointer to the active joystick.
	SDL_Joystick *js;

	//! \brief An index to the SDL joystick which should be made active.
	int32 joy_index;
...
public:
	...
	*** \note This is not called within SingletonInitialize because joystick settings must first be loaded by the ScriptEngine
	**/
	void InitializeJoysticks();

	/** \brief Retrieves user-readable string identifying a specific command
	*** \param command The type of command to retrieve the name for
	*** \return A string representing the name of the command (ex: "Confirm"). Empty string if input argument was invalid
	**/
	static const hoa_utils::ustring& CommandName(INPUT_STANDARD_COMMAND command);
	...
	**/
	//@{
	/** \param command The command to retrieve the name for
	*** \return Name of the key in a std::string format. Empty string if command argument was invalid
	**/
	std::string GetKeyName(INPUT_STANDARD_COMMAND command) const;
...
private:
	...
	//! \brief Holds the translated user strings for every standard command
	static hoa_utils::ustring _command_names[COMMAND_TOTAL];

	//! \brief Holds the current user-defined key settings
	private_input::KeySettings _key;

	//! \brief Holds the current user-defined joystick settings
	private_input::JoystickSettings _joystick;

	//! \brief Any key (or joystick button) pressed
	bool _any_key_press;

	//! \brief Any unmapped key pressed
	bool _unmapped_key_press;

	//! \brief Any key released
	bool _any_key_release;

	//! \brief Any joystick axis moved
	...
	**/
	...
	//! \brief Hold the most recent SDL event that was generated
	SDL_Event _event;
