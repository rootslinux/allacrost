...
*** \author Mois�s Ferrer Serra (byaku)
...
void AudioDescriptor::_Update() {
	...
	if (_stream == nullptr || _state != AUDIO_STATE_PLAYING)
		return;
...
void AudioDescriptor::_PrepareStreamingBuffers() {
	if (_stream == nullptr) {
		IF_PRINT_WARNING(AUDIO_DEBUG) << "_stream pointer was nullptr, meaning this function should never have been called" << endl;
		return;
	}
...
void MusicDescriptor::Play() {
	...
	else if (AudioManager->_active_music != nullptr) {
