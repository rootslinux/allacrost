...
void SaveMode::SetCommandDescriptions() {
	...
	_command_descriptions[CONFIRM_COMMAND] = UTranslate("Select");
	_command_descriptions[CANCEL_COMMAND] = UTranslate("Return");
}
...
void SaveMode::Update() {
	...
	if (_current_state == SAVE_MODE_FADING_OUT)
	{
		...
		if (!VideoManager->IsFading()) {
			ModeManager->PopAll();
			try {
				MapMode *MM = new MapMode(MakeStandardString(GlobalManager->GetLocationName()), GlobalManager->GetSaveLoadPoint());
				ModeManager->Push(MM);
...
bool SaveMode::_PreviewGame(int32 id) {
	...
	string location_filename = file.ReadString("map_script_filename");
	...
	ustring location_name = MakeUnicodeString("");

	// Load map file to get location name
	if (map_file.OpenFile(location_filename) == false) {
		if (GLOBAL_DEBUG) {
			PRINT_WARNING << "couldn't open map file, might already be open" << endl;
		}
	} else {
		// Determine the map's tablespace name and then open it. The tablespace is the name of the map file without
		...
		location_name = MakeUnicodeString(map_file.ReadString("map_name"));
...
SmallCharacterWindow::SmallCharacterWindow() : _character(nullptr) {
}
