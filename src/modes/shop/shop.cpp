...
StillImage* GetStatusIcon(GLOBAL_STATUS status_type, GLOBAL_INTENSITY intensity) {
	...
	PRINT_WARNING << "called unimplement function: " << status_type << ", " << intensity << endl;
	return nullptr;
}
...
void ShopObjectViewer::_SetEquipmentData() {
	...
	else if (selected_armor != nullptr) {
	...
	GlobalWeapon* equipped_weapon = nullptr;
	GlobalArmor* equipped_armor = nullptr;
...
void ShopObjectViewer::_SetStatusIcons(const map<GLOBAL_STATUS, GLOBAL_INTENSITY>& status_effects) {
	...
	PRINT_WARNING << "called unimplemented function: " << status_effects.size() << endl;
}
...
void ShopMode::SetCommandDescriptions() {
...
void ShopMode::Update() {
	...
	if (_state == SHOP_STATE_ROOT) {
		SoundDescriptor* sound = nullptr; // Used to hold pointers of sound objects to play
...
void ShopMode::AddObjectToBuyList(ShopObject* object) {
	if (object == nullptr) {
		IF_PRINT_WARNING(SHOP_DEBUG) << "function was passed a nullptr argument" << endl;
		return;
	}
