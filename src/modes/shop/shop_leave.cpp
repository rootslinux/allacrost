...
LeaveInterface::LeaveInterface() :
	...
	_trade_count(0),
	_trade_characters(0),
	_buy_list_display(nullptr),
	_sell_list_display(nullptr)
{
...
LeaveInterface::~LeaveInterface() {
	if (_buy_list_display != nullptr)
		delete _buy_list_display;
	if (_sell_list_display != nullptr)
...
void LeaveInterface::MakeActive() {
	...
	ShopMode::CurrentInstance()->ObjectViewer()->SetSelectedObject(nullptr);
...
void LeaveInterface::_ChangeState(LEAVE_STATE new_state) {
	...
	if (new_state == LEAVE_STATE_MAIN) {
		...
		ShopObject* selected_object = nullptr;
		switch (_active_list) {
			case ACTIVE_LIST_BUY:
