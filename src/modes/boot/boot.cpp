	...
	_file_name_window(ustring(),150.0f,35.0f)
{
	...
	// TODO: this will need to pull the official release version number
	...
	_copyright_text.SetText(UTranslate("Copyright 2004-2018, The Allacrost Project"));
...
void BootMode::SetCommandDescriptions() {
	...
	_command_descriptions[CONFIRM_COMMAND] = UTranslate("Select menu");
...
void BootMode::Update() {
	...
	// If an unmapped key is pressed, show the help menu so long as none of the keyboard/joystick setting menus are not active
	if (InputManager->UnmappedKeyPress() == true) {
        if (_active_menu != &_key_settings_menu && _active_menu != &_joy_settings_menu) {
			ModeManager->Push(new PauseMode(hoa_pause::HELP));
		}
	}
	...
	if (_overwrite_function != nullptr)
	{
...
void BootMode::_SetupVideoOptionsMenu() {
	...
	_video_options_menu.AddOption(UTranslate("Window mode:"), &BootMode::_OnToggleFullscreen, nullptr, nullptr, &BootMode::_OnToggleFullscreen, &BootMode::_OnToggleFullscreen);
	_video_options_menu.AddOption(UTranslate("Brightness:"), nullptr, nullptr, nullptr, &BootMode::_OnBrightnessLeft, &BootMode::_OnBrightnessRight);
...
void BootMode::_SetupAudioOptionsMenu() {
	...
	_audio_options_menu.AddOption(UTranslate("Sound Volume: "), nullptr, nullptr, nullptr, &BootMode::_OnSoundLeft, &BootMode::_OnSoundRight);
	_audio_options_menu.AddOption(UTranslate("Music Volume: "), nullptr, nullptr, nullptr, &BootMode::_OnMusicLeft, &BootMode::_OnMusicRight);
...
void BootMode::_OnLoadGame() {
	// If the Load Game menu option is disabled, do nothing other than play a sound to indicate it is unavailable
	if (_main_menu.IsOptionEnabled(1) == false) {
		_boot_sounds.at(3).Play();
	}
	else {
		_boot_music.at(0).Stop();
...
//	Effects::FadeIn(_boot_music.at(0), 50.0f);
	...
	if (settings_lua.ReadInt("welcome") == 1) {
		ModeManager->Push(new PauseMode(hoa_pause::HELP));
		// Save the settings as this will automatically change the welcome setting to 0
		_has_modified_settings = true;
		_SaveSettingsFile("");
	}
