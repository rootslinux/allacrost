...
public:
	...
	**/
	...
	void SetPosition(uint16 x, float x_offset, uint16 y, float y_offset)
		{ x_position = x; this->x_offset = x_offset; y_position = y; this->y_offset = y_offset; CheckPositionOffsets(); }
...
protected:
	...
	/** \brief Draws a translucent colored area representing the object's collision rectangle
	***
	*** This is only used for debugging to make it easier to visualize things like pathing issues or
	*** problems with collision resolution. This function should only be called by a map object's draw
	*** function as it does not set the draw position coordinates and does not check if the object is visible
	*** on the current context and screen. Also note that the box will not be drawn if the object's collidable
	*** property is disabled.
	**/
	void DEBUG_DrawCollisionBox();
}; // class MapObject
...
public:
	...
	*** \return A pointer to the object at this index, or nullptr if no object exists at the given index
	***
	...
	**/
	ObjectLayer* GetObjectLayer(uint32 layer_id)
		{ if (layer_id >= _object_layers.size()) return nullptr; else return &_object_layers[layer_id]; }
	...
	*** \return A pointer to the object occupying the grid position or nullptr if the position is unoccupied
	***
	...
	*** This member may be set to nullptr if this information is not required by the callee. Otherwise, the callee
	...
	*** \return The type of collision detected (NO_COLLISION if none was detected)
	***
	*** This method is invoked by a map sprite who wishes to check to see if it colliding with anything when moving to a new position.
	...
	**/
	...
	/** \brief Draws a colored highlight over every unit of the visible collision grid to indicate which areas are collidable
	*** \param context The active context of the map being drawn
	***
	*** This function is called when debugging is enabled and helps ensure that things like movement, collision resolution, and pathfinding are working
	*** properly. It uses a translucent red color to highlight the collidable areas of the map. And, as expected, when the map context is changed so does
	*** the highlighted collision area to reflect the new context.
	**/
	void DEBUG_DrawCollisionGrid(MAP_CONTEXT context);

	/** \brief Used in graphical debugging. Draws a colored outline for all visible map zones on the active context
	*** \param context Zones must exist in this context in order for their outlines to be drawn
	**/
	void DEBUG_DrawZoneOutlines(MAP_CONTEXT context);
