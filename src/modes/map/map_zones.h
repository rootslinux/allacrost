...
public:
	MapZone() : _zone_id(0), _active_contexts(MAP_CONTEXT_NONE)
		{}
	...
	**/
	...
	/** \brief Draws an outline for each zone section that is visible on the screen
	*** \param context The active map context that the zone must exist for the outline to be drawn
	**/
	virtual void DEBUG_DrawZoneOutlines(MAP_CONTEXT context)
		{ DEBUG_DrawZoneOutlines(context, ZONE_OUTLINE_COLOR); }
...
protected:
	//! \brief The color for drawing zone outlines for graphical debugging (blue)
	static const hoa_video::Color ZONE_OUTLINE_COLOR;

	//! \brief Used to identify the zone (optional; 0 by default)
	uint32 _zone_id;
	...
	**/
	...
	/** \brief Draws an outline for each zone section that is visible on the screen
	*** \param context The active map context that the zone must exist for the outline to be drawn
	*** \param color The color to draw the outline (default color == blue)
	**/
	void DEBUG_DrawZoneOutlines(MAP_CONTEXT context, const hoa_video::Color& color);
}; // class MapZone
...
public:
	...
	**/
	bool IsSpriteExiting(uint32 object_id) const;
	...
	*** \return A pointer to the sprite, or nullptr if there is no resident for the given index
	***
	...
	*** \return A pointer to the sprite, or nullptr if there is no entering resident for the given index
	***
	...
	*** \return A pointer to the sprite, or nullptr if there is no exiting resident for the given index
	***
...
protected:
	...
	*** \return A pointer to the sprite at the specified index, or nullptr if no sprite exists at that index
	**/
...
***
*** \todo Currently no context data is used for enemy zones. The constructors set the zone
*** to be available in "all contexts". We need to figure out whether or not this makes sense.
*** ***************************************************************************/
...
public:
	...
	***
	*** This is commonly used on map startup to populate the map with enemies.
	*** \note This function ignores if spawning has been disabled by the zone.
	**/
	...
	/** \brief Draws an outline for each zone section that is visible on the screen
	*** \param context The active map context that the zone must exist for the outline to be drawn
	*** \param color The color to draw the outline (default color == purple)
	*** \note Enemy zones are drawn in a different default color the other zones to distinguish them
	**/
	void DEBUG_DrawZoneOutlines(MAP_CONTEXT context, const hoa_video::Color& color = ENEMY_ZONE_OUTLINE_COLOR)
		{ MapZone::DEBUG_DrawZoneOutlines(context, color); }

	/** \brief Draws an outline for each zone section that is visible on the screen
	*** \param context The active map context that the zone must exist for the outline to be drawn
	*** \note Enemy zones distinguish themselves from other zones drawn by using a different purple color
	**/
	virtual void DEBUG_DrawZoneOutlines(MAP_CONTEXT context)
		{ DEBUG_DrawZoneOutlines(context, ENEMY_ZONE_OUTLINE_COLOR); }
	...
	bool IsSpawningDisabled() const
		{ return _spawning_disabled; }
	...
	void SetSpawningDisabled(bool disabled)
...
protected:
	//! \brief A separate graphical debugging color for enemy zones (purple)
	static const hoa_video::Color ENEMY_ZONE_OUTLINE_COLOR;

private:
	//! \brief If true, enemies of this zone are not allowed to roam outside of the zone boundaries
	bool _roaming_restrained;

	//! \brief While true, no enemies may spawn within this zone
	bool _spawning_disabled;
