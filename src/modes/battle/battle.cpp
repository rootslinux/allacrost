...
// 		PRINT_ERROR << "failed to load swap card" << endl;
	...
	if (ImageDescriptor::LoadMultiImageFromElementSize(_target_type_icons, "img/icons/effects/targets.png", 25, 25) == false)
		PRINT_ERROR << "failed to load target type icons image" << endl;
...
void BattleMode::SetCommandDescriptions() {
	...
	_command_descriptions[CONFIRM_COMMAND] = UTranslate("Select action");
	_command_descriptions[CANCEL_COMMAND] = UTranslate("Return to previous menu");
	_command_descriptions[MENU_COMMAND] = UTranslate("View detail about selection");
	_command_descriptions[SWAP_COMMAND] = UTranslate("Hold to view character status");
}
...
void BattleMode::Update() {
	...
	// If the battle is in its typical state and player is not selecting a command, check for player input
		...
		if (character_selection != nullptr) {
...
void BattleMode::_DrawSprites() {
	// Booleans used to determine whether or not the actor selector graphic should be drawn
	...
	BattleActor* actor_target = target.GetActor(); // A pointer to an actor being targetted (value may be nullptr if target is party)

	// Determine if selector graphics should be drawn
	if ((_state == BATTLE_STATE_COMMAND) && (_command_supervisor->GetState() == COMMAND_STATE_ACTOR)) {
