...
void DialogueSupervisor::AddCharacterSpeaker(uint32 id, BattleCharacter* character) {
	if (character == nullptr) {
		IF_PRINT_WARNING(BATTLE_DEBUG) << "function received nullptr character argument" << endl;
		return;
	}
...
void DialogueSupervisor::_UpdateLine() {
	...
	if (_current_dialogue->IsHaltBattleAction() == false || _current_options != nullptr || _dialogue_window.GetDisplayTextBox().IsFinished() == false) {
