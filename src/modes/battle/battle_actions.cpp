...
{
	if (actor == nullptr)
		IF_PRINT_WARNING(BATTLE_DEBUG) << "constructor received nullptr actor" << endl;
...
{
	if (skill == nullptr) {
		IF_PRINT_WARNING(BATTLE_DEBUG) << "constructor received nullptr skill argument" << endl;
		return;
	}
...
bool SkillAction::Execute() {
	...
	if (_target.IsValid() == false) {
		// TEMP: party targets should always be valid, so only the actor needs to be changed
...
{
	if (item == nullptr) {
		IF_PRINT_WARNING(BATTLE_DEBUG) << "constructor received nullptr item argument" << endl;
		return;
	}
