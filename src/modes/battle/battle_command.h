...
public:
	...
	*** \return A pointer to the item, or nullptr if the item_index argument was out-of-range
	**/
	BattleItem* GetItem(uint32 item_index)
		{ if (item_index >= _items.size()) return nullptr; else return &(_items[item_index]); }
...
public:
	...
	**/
	...
	BattleCharacter* GetCommandCharacter() const
		{ if (_active_settings == nullptr) return nullptr; else return _active_settings->GetCharacter(); }
...
private:
	...
	**/
	...
	bool _HasCharacterSettings(BattleCharacter* character) const
		{ if (character == nullptr) return false; else return (_character_settings.find(character) != _character_settings.end()); }
