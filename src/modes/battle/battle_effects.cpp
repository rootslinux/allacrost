...
{
	if (actor == nullptr) {
		IF_PRINT_WARNING(BATTLE_DEBUG) << "constructor received nullptr actor argument" << endl;
		return;
	}
}
...
StatusEffect::StatusEffect(GLOBAL_STATUS type, GLOBAL_INTENSITY intensity, BattleActor* actor) :
	...
	_duration_timer(0),
	_icon_image(nullptr),
	_apply_function(nullptr),
	_update_function(nullptr),
	_remove_function(nullptr)
{
...
{
	if (actor == nullptr)
		IF_PRINT_WARNING(BATTLE_DEBUG) << "contructor received nullptr actor argument" << endl;
}
	...
	GLOBAL_STATUS& new_status, GLOBAL_INTENSITY& new_intensity)
{
	...
	StatusEffect* active_effect = nullptr;
...
void EffectsSupervisor::_CreateNewStatus(GLOBAL_STATUS status, GLOBAL_INTENSITY intensity) {
	...
	StatusEffect* new_effect = nullptr;
...
void EffectsSupervisor::_RemoveStatus(StatusEffect* status_effect) {
	if (status_effect == nullptr) {
		IF_PRINT_WARNING(BATTLE_DEBUG) << "function received nullptr status_effect argument" << endl;
		return;
	}
